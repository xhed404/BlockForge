...
if ($z) { $zlib = $z.FullName }
if ([string]::IsNullOrWhiteSpace($zlib)) { throw "zlib1.dll not found under build dir $buildDirAbs (installer would not run)" }
Copy-Item $zlib (Join-Path $appDir "zlib1.dll") -Force
