    ...
    : QMainWindow(parent)
{
    ...
    setStyleSheet(
        "QMainWindow{background:#0b0f14;color:#e7e7e7;}"
        ...
        "QTreeWidget{background:#0f151d;border:1px solid #202833;border-radius:10px;}"
    ...
    auto* brand = new QLabel("BlockForge", sidebar);
    QFont bf = brand->font();
    bf.setPointSize(14);
    bf.setBold(true);
    brand->setFont(bf);
    ...
    auto* topBar = new QWidget(pageInstances);
    auto* topBarLayout = new QHBoxLayout(topBar);
    topBarLayout->setContentsMargins(16, 16, 16, 0);
    ...
    auto* btnExport = new QPushButton("Export", topBar);
    ...
    auto* split = new QSplitter(pageInstances);

    auto* list = new QTreeWidget(split);
    ...
    auto* title = new QLabel("Select an instance", detail);
    QFont tf = title->font();
    tf.setPointSize(16);
    tf.setBold(true);
    title->setFont(tf);
    ...
    instancesLayout->addWidget(topBar);
    instancesLayout->addWidget(split, 1);
    ...
    connect(addAccount, &QPushButton::clicked, this, [=]() {
        ...
        auto* thread = new QThread(logsView);
