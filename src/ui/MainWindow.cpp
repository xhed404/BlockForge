    ...
    : QMainWindow(parent)
{
    ...
    auto* statusNetwork = new QLabel("Network: <span style=\"color:#44d07a;\">●</span> Online", this);
    statusNetwork->setStyleSheet("color:#aab2bd;");
    auto* statusAuth = new QLabel("Auth: <span style=\"color:#737b86;\">●</span> Offline", this);
    ...
    setStyleSheet(
        ...
        "QLineEdit,QPlainTextEdit,QComboBox{background:#121820;border:1px solid #2a3440;border-radius:6px;padding:6px;min-height:34px;}"
        "QComboBox::drop-down{border:0px;}"
        "QPushButton{background:#1a232e;border:1px solid #2a3440;border-radius:8px;padding:8px 12px;min-height:34px;}"
        ...
        "QToolButton{background:transparent;border:0px;padding:12px 10px;text-align:left;color:#c9ced6;}"
        "QToolButton:hover{background:#0f151d;}"
        "QToolButton:checked{background:#141a22;border-left:3px solid #ff7a18;color:#ff7a18;}"
        ...
        "QHeaderView::section{background:#0f151d;border:0px;padding:10px 8px;color:#aab2bd;}"
    ...
    auto* profileTitle = new QLabel("Profile", detail);
    ...
    loaderType->addItems(QStringList() << "Vanilla"
                                       << "Fabric"
                                       << "Forge");
    ...
    javaVersion->addItems(QStringList() << "Auto"
                                        << "Java 8 (Temurin)"
                                        << "Java 17 (Temurin)"
                                        << "Java 21 (Temurin)");
    ...
    formLayout->addRow(makeLabel("Minecraft version"), mcVersion);
    formLayout->addRow(makeLabel("Loader"), loaderRow);
    formLayout->addRow(makeLabel("Java version"), javaVersion);
    formLayout->addRow(makeLabel("Java path"), javaRow);
    formLayout->addRow(makeLabel("RAM Allocation"), ramBox);
    formLayout->addRow(makeLabel("Notes"), notesBox);
    ...
    auto loaderDisplay = [](const Instance& inst) {
        const auto lt = QString::fromStdString(toString(inst.loaderType));
    ...
    auto updateDetails = [=]() {
        ...
        if (inst.loaderType == LoaderType::Vanilla) loaderType->setCurrentText("Vanilla");
        else if (inst.loaderType == LoaderType::Fabric) loaderType->setCurrentText("Fabric");
        else loaderType->setCurrentText("Forge");
        ...
        javaVersion->setCurrentText(QString::fromStdString(settings.get(instanceKey(*selectedInstanceId, "javaVersion")).value_or("Auto")));
    ...
    auto refreshAccounts = [=]() {
        ...
        statusAuth->setText(activeIndex >= 0 ? "Auth: <span style=\"color:#44d07a;\">●</span> Signed in"
    ...
    connect(loaderType, &QComboBox::currentTextChanged, this, [=](const QString& t) {
        if (selectedInstanceId->isEmpty()) return;
        auto it = instancesById->find(selectedInstanceId->toStdString());
        if (it == instancesById->end()) return;

        LoaderType lt = LoaderType::Vanilla;
        if (t == "Fabric") lt = LoaderType::Fabric;
        else if (t == "Forge") lt = LoaderType::Forge;
        it->second.loaderType = lt;

        writeInstanceMeta(*selectedInstanceId, "loaderType", t.toLower(), false);
    ...
    connect(javaVersion, &QComboBox::currentTextChanged, this, [=](const QString& t) {
        if (selectedInstanceId->isEmpty()) return;
        auto settings = appSettings();
        settings.set(instanceKey(*selectedInstanceId, "javaVersion"), t.trimmed().toStdString());
    });
