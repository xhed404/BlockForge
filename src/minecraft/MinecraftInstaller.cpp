...
void MinecraftInstaller::setProgressCallback(std::function<void(const QString& phase, int current, int total)> cb)
{
...
VersionInstallResult MinecraftInstaller::downloadAssets(const QString& versionId, int limit)
{
    ...
    for (auto it = objects.begin(); it != objects.end(); ++it)
    {
        ...
        const auto size = static_cast<qint64>(obj.value("size").toDouble(0));
        if (hash.size() < 2)
        {
            continue;
        }
        ...
        if (fi.exists() && fi.size() == size && size > 0)
        {
            continue;
        }
        ++needTotal;
        if (limit > 0 && needTotal >= limit)
        {
            break;
        }
    }
    ...
    int done = 0;
    if (m_progress) m_progress("assets", 0, needTotal);
    for (auto it = objects.begin(); it != objects.end(); ++it)
    {
        ...
        ++done;
        if (m_progress && ((done % 32) == 0 || done == needTotal)) m_progress("assets", done, needTotal);
    }
    if (m_progress) m_progress("assets", done, needTotal);
